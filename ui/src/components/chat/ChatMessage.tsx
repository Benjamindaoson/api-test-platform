...
export const ChatMessage = React.memo<ChatMessageProps>(
          ...
          .map(
            (tc: ToolCall) =>
              ({
                id: tc.id,
                name: tc.name,
                subAgentName: (tc.args as Record<string, unknown>)[
                  "subagent_type"
                ] as string,
                input: tc.args,
                output: tc.result ? { result: tc.result } : undefined,
                status: tc.status === "pending" ? "active" : tc.status,
              } as SubAgent),
          ),
      [toolCalls],
    );
    ...
    return (
      ...
      >
        ...
        >
          ...
          {!isUser && subAgents.length > 0 && !hideToolCalls && (
  ...
  (prevProps, nextProps) => {
    ...
    const isSameLastMessageState =
      ...
      prevProps.hideToolCalls === nextProps.hideToolCalls;
